...
- **轻**：LangGraph 核心只做图编排；LLM 调用改用 LangChain 官方维护的 `ChatDeepSeek`（`langchain-deepseek`），并直接复用 LangGraph 预构建的 `create_react_agent` + `ToolNode`，不再自研 OpenAI 兼容客户端与 ReAct 循环（详见「改造点 3」）。
...
| LLM 客户端 | 自研 `LLMClient`（requests） | `ChatDeepSeek`（langchain-deepseek） |
| ReAct 运行时 | 自研 `run_agent` 循环 | LangGraph `create_react_agent` + `ToolNode` |
| 依赖 | `requests` / `PyYAML` | + `langgraph`、`langchain-deepseek` |
...
#### 改造点 3 详细设计：LLM 客户端 + ReAct 运行时也交给 LangGraph

> 背景：自研 OpenAI 兼容 `LLMClient` 在推理模型、`max_tokens` 截断、`tool_calls` 解析、`finish_reason` 边界上踩坑多（实测 `deepseek-v4-pro` 推理模型因思考占满 `max_tokens` 导致 `content` 为空、`finish_reason=length`）。决定不再自研，改用 LangChain 官方维护的 DeepSeek 集成，并把 ReAct 循环也交给 LangGraph 预构建组件。

##### 选型：ChatDeepSeek（langchain-deepseek）

- LangChain 官方维护，基于 `langchain-core`（LangGraph 已自带），专门适配 DeepSeek。
- 正确处理：`reasoning_content`（推理模型）、结构化 `tool_calls`、`usage_metadata`、`max_retries`、`reasoning_effort`（控制推理强度）。
- 调用方式：`llm.invoke(messages)` → `AIMessage`（`.content` / `.tool_calls` / `.usage_metadata`），替代自研 `llm.chat(...)` → `LLMResponse`。

##### LLM 客户端替换（llm.py）

- 删除自研 `LLMClient` / `LLMResponse` / `ToolCall`；`get_llm()` 改为返回 `ChatDeepSeek`（`model`/`api_key`/`api_base`/`temperature`/`max_tokens` 从 `.env` 读取）。
- `dev_agent` 三个 LLM 节点（design/implement/fix）改用 `ChatDeepSeek.invoke`，输出解析从 `resp.content` 改为 `msg.content`。

##### ReAct 运行时替换（runtime.py）

- 自研 `run_agent` 循环替换为 LangGraph 预构建 `create_react_agent(model, tools, state_modifier=prompt)` + `ToolNode`。
- 工具仍来自 `tools.py` 的 `@tool` 注册（函数即 tool），但函数字典需转换为 LangChain `BaseTool`（用 `StructuredTool.from_function` 依据类型注解自动生成 schema），交给 `ToolNode` 执行。
- 智能体包的「内容」（`prompt.md` + `tools.py` + `agent.yaml`）不变，只换运行时引擎；`run_agent` 的返回结构（output/tool_calls/tool_errors/latency/tokens）保持不变，评测器 `evaluator` 无需改动。

##### 关于 reasoning 模型（.env）

- `deepseek-v4-flash` 是推理模型，思考过程占用 `max_tokens` 配额，是空 `content`（`finish_reason=length`）的根因；`deepseek-chat` 已下线。
- 处理：改用 `deepseek-v4-flash` + `ChatDeepSeek(reasoning_effort="low")` 压低思考长度，并把 `max_tokens` 提高到 32768。

##### 影响范围

| 文件 | 变更 |
|---|---|
| `llm.py` | `LLMClient` → `ChatDeepSeek` |
| `runtime.py` | 自研 ReAct 循环 → `create_react_agent` + `ToolNode` |
| `dev_agent.py` | `_chat` / `smoke_test` 适配 `AIMessage` 与新版运行时 |
| `tools.py` | `@tool` 注册不变；新增「函数 → LangChain BaseTool」转换 |
| `evaluator.py` | 不变（依赖 `run_agent` 的返回结构） |
...
- **LLM 调用日志独立落盘**：每次 LLM 请求（messages 全文）与输出（content/tool_calls/usage）记录到独立 logger `alip.llm`，写入 `data/logs/llm_calls.log`；该 logger `propagate=False`，**不进入 platform.log**，便于单独排查请求/输出而不污染平台日志。
...
requests>=2.31
PyYAML>=6.0
langgraph>=0.2            # 状态图编排 + 预构建 ReAct agent（create_react_agent/ToolNode）
langchain-deepseek>=1.0   # DeepSeek 专用 LLM 客户端（替代自研 LLMClient）
```

> 说明：`langgraph` 会自带 `langgraph-checkpoint`、`langchain-core` 等传递依赖；`langchain-deepseek` 依赖 `langchain-core`，共同构成运行 LangGraph + DeepSeek 的最小集合，不主动使用 LangChain 的 chain/prompt 抽象。

---

## 改造点清单

1. **依赖**：`requirements.txt` 增加 `langgraph` + `langchain-deepseek`，验证可安装、可 import。
2. **替换 LLM 客户端**：`llm.py` 删除自研 `LLMClient`，`get_llm()` 返回 `ChatDeepSeek`。
3. **替换 ReAct 运行时**：`runtime.py` 自研循环改为 `create_react_agent` + `ToolNode`，`tools.py` 增加「函数 → LangChain BaseTool」转换。
4. **定义状态与图**：`alip/dev_agent.py`（或 `alip/dev/` 包）实现 `DevState`、`StateGraph`、条件边与重试上限（`attempts`）。
5. **内置语法门禁**：`syntax_check` 节点用 `py_compile` 检查 `tools.py`，失败带报错回环修复。
6. **内置冒烟测试**：`smoke_test` 节点复用 `load_package` + `run_agent`（改造后的 create_react_agent 版），开发与测试合并在图内。
7. **重构 CLI**：`cli.cmd_create` / `_create_agent` 改为"启动 LangGraph 图 → 成功出口落盘并注册"，删除独立冒烟分支。
8. **持久化日志**：`logging` 双 Handler（控制台 INFO / 文件 DEBUG），每个节点与每轮重试落 DEBUG 全流程。
9. **reasoning 模型处理**：`deepseek-chat` 已下线；`.env` 用 `deepseek-v4-flash`，`get_llm()` 设 `reasoning_effort="low"` + `max_tokens=32768`。
10. **LLM 调用日志**：`alip.llm` 独立 logger 记录每次 LLM 请求/输出到 `data/logs/llm_calls.log`，`propagate=False` 不进入 platform.log。
11. **验收**：`python main.py create "开户合规智能体"` 一条命令稳定产出**通过语法 + 冒烟**的智能体包并注册入库；`data/logs/` 下可见完整 debug 日志与 `llm_calls.log`；失败时能回放定位到具体节点与第几轮。
